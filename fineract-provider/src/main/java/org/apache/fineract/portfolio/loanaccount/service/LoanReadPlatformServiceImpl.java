...
public class LoanReadPlatformServiceImpl implements LoanReadPlatformService {
		...
		public LoanAccountData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum)
			...
			if (status.id().intValue() >= 300) {
				...
				loanSummary = new LoanSummaryData(currencyData, principalDisbursed, principalPaid, principalWrittenOff,
						...
						totalCostOfLoan, totalWaived, totalWrittenOff, totalOutstanding, totalOverdue,
						overdueSinceDate);
			}
			...
			return LoanAccountData.basicLoanDetails(id, accountNo, status, externalId, clientId, clientAccountNo,
					...
					transactionStrategyName, amortizationType, interestRatePerPeriod, flatInterestRatePerPeriod,
					interestRateFrequencyType, annualInterestRate, interestType, isFloatingInterestRate,
					interestRateDifferential, advanceEmiN, interestCalculationPeriodType,
					...
					syncDisbursementWithMeeting, loanCounter, loanProductCounter, multiDisburseLoan,
					canDefineInstallmentAmount, fixedEmiAmount, outstandingLoanBalance, inArrears, graceOnArrearsAgeing,
			...
			if (transactionType.isPaymentOrReceipt()) {
				...
				if (paymentTypeId != null) {
					...
					paymentDetailData = new PaymentDetailData(id, paymentType, accountNumber, checkNumber, routingCode,
							receiptNumber, bankNumber, chequeDate, bankName);
				}
			}
	...
	public LoanTransactionData retrieveLoanWriteoffTemplate(final Long loanId) {
		...
		final LoanTransactionEnumData transactionType = LoanEnumerations.transactionType(LoanTransactionType.WRITEOFF);
		final BigDecimal unrecognizedIncomePortion = null;
		return new LoanTransactionData(null, null, null, transactionType, null, loan.currency(),
				DateUtils.getLocalDateOfTenant(), loan.getTotalOutstandingAmount(), null, null, null, null, null, null,
				null, null, outstandingLoanBalance, unrecognizedIncomePortion, false);
	}
